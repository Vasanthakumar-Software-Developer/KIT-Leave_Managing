...
class Submit2 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val button = findViewById<Button>(R.id.Submit21)
        val add = findViewById<Button>(R.id.add2)
            ...
            if (name.isNotEmpty() && year.text.isNotEmpty() && dep.text.isNotEmpty() && per.text.isNotEmpty() && noper.isNotEmpty()) {
                ...
                database.child("$dep1".toUpperCase()).child("$name1-$str").child("Name")
                    .setValue("$name1").addOnSuccessListener {
                        ...
                        database.child("$dep1".toUpperCase()).child("$name1-$str")
                            .child("Destination")
                        ...
                        database.child("$dep1".toUpperCase()).child("$name1-$str").child("Inform")
                            .setValue("$inform")
                        database.child("$dep1").child("$name1-$str").child("AName")
                            .setValue("$name")
                        ...
                        database.child("$dep1").child("$name1-$str").child("Aperiods")
                            .setValue("${per.text}")
                        database.child("$dep1").child("$name1-$str").child("date")
                            .setValue("$date")
                        ...
                        if ("$dep1".equals("${dep.text}")) {
                            ...
                            database.child("${dep.text}".toUpperCase()).child("$name1-$date")
                                .child("Name")
                                .setValue("$name1").addOnSuccessListener {
                                    database.child("${dep.text}".toUpperCase())
                                        .child("$name1-$date").child("Deparment")
                                        .setValue("$dep1")
                                    database.child("${dep.text}".toUpperCase())
                                        .child("$name1-$date").child("Destination")
                                        .setValue("$destination")
                                    database.child("${dep.text}".toUpperCase())
                                        .child("$name1-$date").child("Purpose")
                                        .setValue("$purpose")
                                    database.child("${dep.text}".toUpperCase())
                                        .child("$name1-$date").child("NoofDays")
                                        .setValue("$day")
                                    database.child("${dep.text}".toUpperCase())
                                        .child("$name1-$date").child("FromToDate")
                                        .setValue("$fdate")
                                    database.child("${dep.text}".toUpperCase())
                                        .child("$name1-$date").child("LeaveType")
                                        .setValue("$leavetype")
                                    database.child("${dep.text}".toUpperCase())
                                        .child("$name1-$date").child("Inform")
                                        .setValue("$inform")
                                    database.child("${dep.text}").child("$name1-$date")
                                        .child("AName").setValue("$name")
                                    database.child("${dep.text}").child("$name1-$date")
                                        .child("AYear")
                                        .setValue("${year.text}")
                                    database.child("${dep.text}").child("$name1-$date")
                                    ...
                                    database.child("${dep.text}").child("$name1-$date")
                                        .child("Anoofhours")
                                        .setValue("$noper")
                                    database.child("${dep.text}").child("$name1-$date").child("date").setValue("$date")
        ...
        add.setOnClickListener {
            ...
            if (name.isNotEmpty() && year.text.isNotEmpty() && dep.text.isNotEmpty() && per.text.isNotEmpty() && noper.isNotEmpty()) {
                ...
                if ("$dep1".equals("${dep.text}")) {
                    ...
                    database.child("${dep.text}".toUpperCase()).child("$name1-$date").child("Name").setValue("$name1").addOnSuccessListener {
                            database.child("${dep.text}".toUpperCase()).child("$name1-$date").child("Deparment").setValue("$dep1")
                            database.child("${dep.text}".toUpperCase()).child("$name1-$date").child("Destination").setValue("$destination")
                            database.child("${dep.text}".toUpperCase()).child("$name1-$date").child("Purpose").setValue("$purpose")
                            database.child("${dep.text}".toUpperCase()).child("$name1-$date").child("NoofDays").setValue("$day")
                            database.child("${dep.text}".toUpperCase()).child("$name1-$date").child("FromToDate").setValue("$fdate")
                            database.child("${dep.text}".toUpperCase()).child("$name1-$date").child("date").setValue("$date1")
                            database.child("${dep.text}".toUpperCase()).child("$name1-$date").child("LeaveType").setValue("$leavetype")
                            database.child("${dep.text}".toUpperCase()).child("$name1-$date").child("Inform").setValue("$inform")
                            database.child("${dep.text}").child("$name1-$date").child("AName").setValue("$name")
                            database.child("${dep.text}").child("$name1-$date").child("AYear").setValue("${year.text}")
                            database.child("${dep.text}").child("$name1-$date").child("Adeparment").setValue("${dep.text}")
                            database.child("${dep.text}").child("$name1-$date").child("Anoofhours").setValue("$noper")
                            database.child("${dep.text}").child("$name1-$date").child("Aperiods").setValue("${per.text}")
                            database.child("${dep.text}").child("$name1-$date").child("date").setValue("$date1")
    ...
    private fun yearselection() {
        ...
        val Mbuilder = AlertDialog.Builder(this@Submit2)
